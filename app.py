...
from PyQt6.QtCore import Qt, QSSize, pyqtSignal, QEEvent, QThread
from PyQt6.QtGui import QIcon, QTextCursor
from chatgpt import ChatGPT
from db import ChatGPTDatabase

def current_timestamp(formate_pattern='%y_%m_%d_%H%M%S'):
...
try:
    while response is None:
        ...
        if 'error' in response:
            ...
            self.parent.conversation_window.insertHtml('<p style="color:#fd9620"> <strong>[AI')
            self.parent.conversation_window.insertHTml(markdown_converted)
            self.parent.conversation_window.insertHTml('<br')
            self.parent.conversation_window.insertHTml('<br')
            self.parent.status.showMessafe('Tokens used: {0}'.format(response['usage']))
...
# temperature slider
        ...
        self.layout['slider_;ayout2'].addWidget(self.temperature)
...
self.layout['main'].addWidget(splitter)
    ...
    splitter.addWidget(self.conversation_window)
        ...
        self.layout['input entry'] = QHBoxLayout(self.input_window)
    ...
    self.layout['buttons'].addWidget(self.btn_clear, alignment=Qt.AlighnmentFlag.AlignTop)
...
def init_set_default_setting(self):
    # token slider
    self.max_tokens.setTickPosition(QSlider.TickPostition.TicksBelow)
    self.max_token.setTickInterval(500)
    self.max_tokens.setTracking(True)
    self.max_token_value.setText('{0:,}'.format(self.max_tokens.value()))

    # temperature slider
    self.temperature.setTickPositions(QSlider.TickPostion.TicksBelow)
...
def init_configure_signal(self):
    self.max-tokens.valueChanged.connect(Lambda: self.max_token_value.setText('{0: ,}'.format(self.max_tokens.value())))
...
class TabManager(QTabWidget):
    ...
    def __init__(self,parent=None):
        ...
        self.add_tab-button.clicked.connect(self.plusClicked)
        self.setCornerWidget(self.closeTab)
    ...
    self.setWindowTitle('ChatGPT AI Assistant (By Jie Jenn) v1')
    self.setStyleSheet('''
                 QWidget {
                    font-size: 15px;
                }
...
def init_ui(self):
    ...
    self.Layout(self.layout['main']).addWidget(self.tab_manager)
...
def init_menu(self):
    ...
    file_menu.addAction('&Save log do DB', self.save_conversation_log_to_db)
...
def save_output(self):
    ...
    active_tab.status.showMessage('''File saved at {0}/{1}_Chat Log.txt'''.format(os.getcwd(), time))
...
def save_conversation_log_to_db(self):
    ...
    db.inset_record('message_logs', 'messages, created', values)
